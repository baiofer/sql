-- Creo el esquema para la práctica.

create schema practica;

-- Creo primero las tablas sin Foreings keys (FK) que son grupoEmpresarial,
-- color, aseguradora y moneda. Aunque la entidad color podria desnormalizarse,
-- he decidido mantenerla por atenerme a la normalización, y siempre será una
-- mejora de rendimiento de la BBDD.
...
-- Compruebo que se ha creado bien
--select * from practica.moneda;
...
-- Compruebo que se ha creado bien
-- select * from practica.aseguradora;
...
-- Compruebo que se ha creado bien
-- select * from practica.color;

create table if not exists practica.grupoempresarial (
	id serial primary key,
	nombre varchar(100) not null
);

-- Compruebo que se ha creado bien
-- select * from practica.grupoempresarial;

-- Ahora voy creando el resto de tablas según sus FK. Primero crearé las 
-- tablas marca (que depende de grupo empresarial) y modelo (que depende de marca).

create table if not exists practica.marca (
	id serial primary key,
	id_grupo int not null,
	nombre varchar(70) not null,
	constraint grupo_fk foreign key (id_grupo) references practica.grupoempresarial(id)
);

-- Compruebo que se ha creado bien
-- select * from practica.marca; 

create table if not exists practica.modelo (
	id serial primary key,
	id_marca int not null,
	nombre varchar(70) not null,
	constraint marca_fk foreign key (id_marca) references practica.marca(id)
);
...
create table if not exists practica.coche (
	...
	constraint modelo_fk foreign key (id_modelo) references practica.modelo(id),
	constraint grupo2_fk foreign key (id_grupo) references practica.grupoempresarial(id),
	constraint color_fk foreign key (id_color) references practica.color(id)
);

-- Compruebo que se ha creado bien
-- select * from practica.coche;

-- Ahora creo las tablas revisión y poliza, que dependen de la entidad coche
...
create table if not exists practica.revision (
	...
	constraint moneda_fk foreign key (id_moneda) references practica.moneda(id),
	constraint coche_fk foreign key (matricula) references practica.coche(matricula)
);

-- Compruebo que se ha creado bien
-- select * from practica.revision;

create table if not exists practica.poliza (
	id serial primary key,
	matricula varchar(10),
	numero varchar(50) not null,
	fecha_alta date not null,
	id_aseguradora int not null,
	constraint coche2_fk foreign key (matricula) references practica.coche(matricula),
	constraint aseguradora_fk foreign key (id_aseguradora) references practica.aseguradora(id)
);

-- Compruebo que se ha creado bien
-- select * from practica.poliza;


-- Una vez creada la estructura. Cargo el set de datos.
-- En primer lugar, creo la tabla que llamaré datos_vehiculos

create table if not exists practica.datos_vehiculos (
...
-- select * from practica.datos_vehiculos;

-- Una vez creada la tabla, la poblamos con información.

insert into practica.datos_vehiculos (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Compruebo que se ha poblado bien
-- select * from practica.datos_vehiculos;
	
--Ahora vamos a poblar, primero, tas tablas que no tienen FKs. (moneda, aseguradora, color y
-- grupoempresarial)
	
insert into practica.moneda (nombre)
	select dv.moneda  
	from practica.datos_vehiculos dv 
	group by dv.moneda 
	order by dv.moneda;

-- select * from practica.moneda;
	
insert into practica.aseguradora (nombre)
	select dv.aseguradora  
	from practica.datos_vehiculos dv 
	group by dv.aseguradora 
	order by dv.aseguradora;

-- select * from practica.aseguradora;

insert into practica.color (nombre)
	select dv.color  
	from practica.datos_vehiculos dv 
	group by dv.color 
	order by dv.color;
...
insert into practica.grupoempresarial (nombre)
	select dv.grupo  
	from practica.datos_vehiculos dv 
	group by dv.grupo 
	order by dv.grupo;
...
-- Ahora poblaré las tablas con FKs. Empezaré con la tabla marca.

insert into practica.marca (nombre, id_grupo)
	select dv.marca, g.id
	from practica.datos_vehiculos dv
	inner join practica.grupoempresarial g on dv.grupo = g.nombre
	group by dv.marca, g.id, g.nombre 
	order by dv.marca;

-- select * from practica.marca m;

-- Poblamos el modelo

insert into practica.modelo (nombre, id_marca)
	select dv.modelo, m.id
	from practica.datos_vehiculos dv
	inner join practica.marca m on dv.marca = m.nombre
	group by dv.modelo, m.id 
	order by dv.modelo;

-- select * from practica.modelo m;

-- Con todas estas tablas pobladas, puedo poblar la tabla coche

insert into practica.coche (matricula, fecha_compra, kms_totales, id_grupo, id_modelo, id_color)
	select dv.matricula, dv.fecha_compra, dv.kms_totales, g.id, m.id, c.id 
	from practica.datos_vehiculos dv
	inner join practica.grupoempresarial g on dv.grupo = g.nombre
	inner join practica.modelo m on dv.modelo = m.nombre
	inner join practica.color c on dv.color = c.nombre
	group by dv.matricula, dv.fecha_compra, dv.kms_totales, g.id, m.id, c.id
	order by dv.matricula; 

-- select * from practica.coche c 

-- Y por fin, poblamos las tablas que quedan y que dependen de coche (poliza y revision)

insert into practica.poliza (matricula, numero, fecha_alta, id_aseguradora)
	select dv.matricula, dv.n_poliza, dv.fecha_alta_seguro, a.id
	from practica.datos_vehiculos dv
	inner join practica.aseguradora a on dv.aseguradora = a.nombre
	group by dv.matricula, dv.n_poliza, dv.fecha_alta_seguro, a.id
	order by dv.matricula;

-- select * from practica.poliza p 

insert into practica.revision (kms_revision, importe_revision, fecha_revision, id_moneda, matricula)
	select dv.kms_revision, dv.importe_revision, dv.fecha_revision, m.id, c.matricula 
	from practica.datos_vehiculos dv 
	inner join practica.moneda m on dv.moneda = m.nombre 
	inner join practica.coche c on dv.matricula = c.matricula 
	group by dv.kms_revision, dv.importe_revision, dv.fecha_revision, m.id, c.matricula
	order by matricula;

-- select * from practica.revision r 

-- Ya con las tablas pobladas, realizamos la consulta solicitada. El enunciado de la práctica pide la 
-- consulta con el número de póliza. Como según el set de datos hay cohes con mas de una póliza, he 
-- cogido para el select el número de póliza mínimo para coger un único registro por coche, y que la
-- solución dada, se ciña al enunciado de la práctica.
-- También se da el caso de que un mismo numero de póliza, tiene 2 compañias aseguradoras

select 
	c.matricula as "Matrícula", 
	ma.nombre as "Marca",
	m.nombre as "Modelo",
	g.nombre as "Grupo Empresarial",
	to_char(c.fecha_compra::date, 'DD-MM-YYYY') as "Fecha compra", 
	c.kms_totales as "Kms totales",
	c2.nombre as "Color",
	min(a.nombre) as "Aseguradora",
	po.numero::varchar(20) as "Número póliza",
	to_char(po.fecha_alta::date, 'DD-MM-YYYY') as "Fecha de alta"
from practica.coche c 
inner join practica.modelo m on c.id_modelo = m.id
inner join practica.marca ma on m.id_marca = ma.id
inner join practica.grupoempresarial g on ma.id_grupo = g.id
inner join practica.color c2 on c.id_color = c2.id
inner join (
	select p.matricula as "matricula", min(p.numero::int) as "numero", p.fecha_alta, p.id_aseguradora
	from practica.poliza p 
	group by p.matricula, p.fecha_alta, p.id_aseguradora
	) po on c.matricula = po.matricula
inner join practica.aseguradora a on a.id = po.id_aseguradora
group by c.matricula, ma.nombre, m.nombre, g.nombre, c.kms_totales, c2.nombre, po.numero, po.fecha_alta
